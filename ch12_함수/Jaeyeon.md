...
- 원시 타입 인수는 값 자체가 복사되어 매개변수에 전달되므로 원본이 훼손되지 않지만 객체 타입 인수는 참조 값이 복사되어 전달되므로 내부에서 참조값에 의해 원본 객체가 변경되는 부수 효과가 발생한다.
- 이를 해결하기 위해서는 객체를 깊은 복사하여 불변 객체로 만들어 사용하는 방법이 있다.

## 다양한 함수의 형태
### 즉시 실행 함수
- 익명 즉시 실행 함수는 이름 없이 정의된 함수 리터럴을 즉시 실행하는 방식이다.
```
(function (x, y) {
  console.log(x + y);
})(3, 5); // 8 출력
```
- 여기서 function (x, y) { console.log(x + y); }는 익명 함수이며, (3, 5)를 즉시 실행하기 위해 함수 전체를 괄호 ( ... )로 감싸고 있

- 기명 즉시 실행 함수는 이름을 가진 함수가 즉시 실행되는 방식이다.
```
(function add(x, y) {
  console.log(x + y);
})(3, 5); // 8 출력
```
- 여기서 function add(x, y) { console.log(x + y); }는 기명 함수이며, 즉시 실행된다. 하지만 주의할 점은 이 함수 이름(add)은 함수 내부에서만 사용할 수 있고, 외부에서는 접근할 수 없다.
```
...
### 중첩 함수
- 함수 내부에 정의된 함수를 중첩 함수 또는 내부 함수라고 한다. 그리고 중첩 함수를 포함하는 함수를 외부함수라고 한다.

### 콜백 함수
- 콜백 함수는 다른 함수의 인자로 전달되어 실행되는 함수다.
- 고차 함수는 함수를 인자로 받거나 반환하는 함수다.
- 콜백 함수는 고차 함수에 의해 실행되므로, 고차 함수 개념을 이해하면 콜백 함수 개념도 쉽게 파악할 수 있다.
```
// 이 코드에서 higherOrderFunction은 고차 함수이며, myCallback이 콜백 함수 역할을 한다.
...
```
// forEach는 고차 함수이고, function (num) { console.log(num * 2); }가 콜백 함수이다.
...
### 순수 함수와 비순수 함수
- 순수 함수는 같은 입력에 대해 항상 같은 결과를 반환하며, 외부 상태를 변경하지 않는다.
- 비순수 함수는 외부 상태를 변경하거나, 외부 요소에 의존하여 같은 입력에도 다른 출력을 반환할 수 있다.
- 순수 함수는 예측 가능하고 유지보수가 쉬운 코드를 작성하는 데 유리하다.
...
|---------------------|--------------------------------|----------------------------------|
| **출력 일관성**    | 동일한 입력 → 항상 동일한 출력  | 동일한 입력 → 다른 출력 가능  |
| **외부 상태 변경** | 없음                            | 있음 (전역 변수 수정, API 호출 등) |
| **부작용 (Side Effect)** | 없음                     | 있음                             |
| **예시**          | `Math.max(a, b)`, `Array.map()` | `console.log()`, `fetch()`, `setTimeout()` |


- 참고로 console.log가 비순수함수인 이유는 순수 함수는 내부에서만 동작해야 하지만, 이 함수는 외부 환경(콘솔)에 영향을 주기 때문이다.
- fetch()는 네트워크 상태나 서버의 응답이 달라질 수 있어서 같은 입력이라도 결과가 항상 같지 않아 비순수 함수이다.
- setTimeout() 은 실행 타이밍이 정해지지 않으며, 실행을 예약하고 비동기적으로 실행되어 비순수 함수이다.
